...
* Next, we install `gunicorn` with `pip install gunicorn`. gunicorn is a high performance web server that can run the Flask app in a production environment.
...
#### Part 3: Preparing the Database (If you are using Postgres)

This part is necessary only if you are using Postgres. If you are using sqlite you do not need to do this Part 3.
The advantages of using Postgres in Heroku is that the DB can be connected to through PGAdmin.
